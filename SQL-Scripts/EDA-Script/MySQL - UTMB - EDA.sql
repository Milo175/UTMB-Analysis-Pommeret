...
SELECT
    ...
    AVG(total_minutes) AS average_finish_time_minutes,
    STDDEV_SAMP(total_minutes) AS standard_deviation
...
SELECT
	c.Category,
    SUM(POWER(total_minutes - c.average_minutes, 2)) AS numerator,
    COUNT(*) -1 AS denominator,
    SQRT(SUM(POWER(total_minutes - average_minutes, 2)) / (COUNT(*) -1)) AS standard_deviation
...
SELECT
    ...
    AVG(finished_percentile_gender) AS average_percentile_gender,
    stddev(finished_percentile_overall) AS standard_dev_overall,
    stddev(finished_percentile_gender) AS standard_dev_gender
FROM utmb_results_table_Pommeret_cleaned
GROUP BY Category
...
# DELETE THE YEAR FROM THE RACE TITLE TO GROUP EACH RACE OVER ALL YEARS TOGETHER IN ONE ROW
...
SELECT
    ...
    AVG(z_score_minutes) AS average_z_score
FROM z_score_calculation
WHERE z_score_minutes > 0
GROUP BY Race
ORDER BY count_z_scores DESC, average_z_score DESC;
;
...
SELECT
    SUM((Elevation_gain - avg_Elevation_gain) * (finished_percentile_overall - avg_finished_percentile_overall)) / 
    SQRT(SUM(POWER(Elevation_gain - avg_Elevation_gain, 2)) * SUM(POWER(finished_percentile_overall - avg_finished_percentile_overall, 2))) AS correlation_elevation_results
FROM (
    SELECT 
        Elevation_gain,
...
SELECT
	MIN(finished_percentile_overall) as best_finish,
    MAX(finished_percentile_overall) as worst_finish
FROM utmb_results_table_Pommeret_cleaned
;
...
SELECT 
    days_between_races,
    AVG(finished_percentile_overall) AS percentile_finish_overall
FROM cte
